 ...
 *
 * The mirror of [DtfTaskNavigationHandler]: the work that may walk the project happens here, under
 * a cancellable modal progress, and never during highlighting. Recognising the call is repeated
 * inside the progress rather than carried over from the pass, so no PSI is held between the two.
 */
class TaskTargetsNavigationHandler : GutterIconNavigationHandler<PsiElement> {
    ...
    override fun navigate(event: MouseEvent, element: PsiElement) {
        ...
        DtfTargetPopup.show(event, targets, DtfBundle.message("dtf.popup.task.title", targets.size)) {
            DtfBundle.message("dtf.popup.task.empty")
        }
    }
}
